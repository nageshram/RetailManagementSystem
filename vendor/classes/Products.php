...
class Products
{
	...
	public function getProducts(){
		...
		$q = $this->con->query("SELECT p.product_id, p.product_title, p.product_price, p.product_qty, p.product_desc, p.product_image, p.product_keywords, c.cat_title, c.cat_id, b.brand_id, b.brand_title, v.store_name FROM products p JOIN categories c ON c.cat_id = p.product_cat JOIN brands b ON b.brand_id = p.product_brand JOIN vendor_info v ON v.vendor_id = p.vendor_id WHERE p.vendor_id='$vid'");
		...
		$vendor_info = [];
		$q = $this->con->query("SELECT * FROM vendor_info");
		if ($q->num_rows > 0) {
			while($row = $q->fetch_assoc()){
				$vendor_info[] = $row;
			}
			//return ['status'=> 202, 'message'=> $ar];
			$_DATA['vendor_info'] = $vendor_info;
		}
	...
	public function addProduct($product_name,
		...
		if ($ext == "jpg" || $ext == "jpeg" || $ext == "png") {
			...
			if ($file['size'] > (1024 * 2)) {
				...
				if (move_uploaded_file($file['tmp_name'], $_SERVER['DOCUMENT_ROOT']."/neuStore/product_images/".$uniqueImageName)) {
					
					$q = $this->con->query("INSERT INTO `products`(`product_cat`, `product_brand`, `product_title`, `product_qty`, `product_price`, `product_desc`, `product_image`, `product_keywords` , `vendor_id`) VALUES ('$category_id', '$brand_id', '$product_name', '$product_qty', '$product_price', '$product_desc', '$uniqueImageName', '$product_keywords', '$vendor_id')");
	...
	public function updateStatus($oid=null)
	{
		if($oid!= null)
		{   $e=$this->con->query("SELECT d_status FROM orders WHERE order_id ='$oid'");
			if($e == "Delivered"){
				return['status'=>202, 'message'=>'Product is already Delivered'];
			}
			else{
			$q=$this->con->query("UPDATE orders SET d_status='Delivered'   WHERE order_id ='$oid'");
		    if ($q) {
				return ['status'=> 202, 'message'=> 'Status Updated'];
			}else{
				return ['status'=> 303, 'message'=> 'Failed to run query'];
			}
		}
...
if(isset($_POST['UPDATE_STATUS']))
{
	if(!empty($_POST['oid'])){
		$p= new Products();
		echo json_encode($p->updateStatus($_POST['oid']));
		exit();
	}
	else{
		echo json_encode(['status'=> 303, 'message'=> 'Invalid details']);
