$(document).ready(function(){

	getCustomerOrders();

/*	function getCustomers(){
		$.ajax({
			url : '../vendor/classes/Customers.php',
			method : 'POST',
			data : {GET_CUSTOMERS:1},
			success : function(response){
				
				console.log(response);
				var resp = $.parseJSON(response);
				if (resp.status == 202) {

					var customersHTML = "";

					$.each(resp.message, function(index, value){

						customersHTML += '<tr>'+
									          '<td>#</td>'+
									          '<td>'+value.first_name+' '+value.last_name+'</td>'+
									          '<td>'+value.email+'</td>'+
									          '<td>'+value.mobile+'</td>'+
									          '<td>'+value.address1+'<br>'+value.address2+'</td>'+
									       '</tr>'

					});

					$("#customer_list").html(customersHTML);

				}else if(resp.status == 303){

				}

			}
		})
		
	}
*/
	function getCustomerOrders(){
		$.ajax({
			url : '../vendor/classes/Customers.php',
			method : 'POST',
			data : {GET_CUSTOMER_ORDERS:1},
			success : function(response){
				
				console.log(response);
				var resp = $.parseJSON(response);
				if (resp.status == 202) {

					var customerOrderHTML = "";

					$.each(resp.message, function(index, value){

						customerOrderHTML +='<tr>'+
								              '<td>#</th>'+
								              '<td>'+ value.order_id +'</td>'+
								              '<td>'+ value.product_id +'</td>'+
								              '<td>'+ value.product_title +'</td>'+
								              '<td>'+ value.qty +'</td>'+
								              '<td>'+ value.o_date +'</td>'+
											  '<td>'+ value.o_price +'</td>'+
											  '<td>'+ value.address1+' '+ value.address2 +'</td>'+
								              '<td><a  oid="'+value.order_id+'" class="btn btn-sm btn-warning update_status" style="color=white">update</a> &nbsp; <span>  </span>'+value.d_status+'</td>'+
											  '<td>'+ value.size +'</td>'+
											  '<td>'+ value.payment +'</td>'+
											  '</tr>';

					});

					$("#customer_order_list").html(customerOrderHTML);

				}else if(resp.status == 303){
					$("#customer_order_list").html(resp.message);
				}

			}
		})
		
	}



   $(document.body).on('click', '.update_status', function(){

	var oid = $(this).attr('oid');
	if (confirm("Are you sure to update the delivary status for this order ?")) {
		$.ajax({
			url : '../vendor/classes/Products.php',
			method : 'POST',
			data : {UPDATE_STATUS: 1, oid:oid},
			success : function(response){
				console.log(response);
				var resp = $.parseJSON(response);
				if (resp.status == 202) {
			        getCustomerOrders();
				}else if (resp.status == 303) {
					alert(resp.message);
				}
			}

		});
	}else{
		alert('Cancelled');
	}
	

   });

});