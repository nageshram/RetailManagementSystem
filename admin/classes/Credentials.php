...
class Credentials
{
	...
	public function createAdminAccount($name, $email, $password){
		...
		if ($q->num_rows > 0) {
			...
			$password1 = $password;
			$q = $this->con->query("INSERT INTO `admin`(`name`, `email`, `password`, `is_active`) VALUES ('$name','$email','$password1','0')");
			if ($q) {
	...
	public function loginAdmin($email, $password){
		...
		if ($q->num_rows > 0) {
			...
			{   $id=$row['id'];
				$q = $this->con->query("UPDATE admin SET is_active = '1' WHERE id = '$id'");
	...
	public function updateActive()
	{
		...
		$q = $this->con->query("UPDATE admin SET is_active = '0' WHERE id = '$id'");
		 }
